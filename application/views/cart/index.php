  ...
  <form method="POST" action="<?= base_url() ?>cart/checkout">
    ...
    <div class="content p-2">
      <?php foreach ($product_cart as $idx => $cart) : ?>
        <input type="text" value="<?= $cart->product_id ?>" name="product_id[<?= $idx ?>]">
        <input type="text" value="<?= $cart->id ?>" name="product_cart_id[<?= $idx ?>]">
        ...
        <div class="card card-product-cart mb-2 flex-row">
          ...
          <div class="col p-2 px-1 d-flex flex-column justify-content-between">
            ...
            <div class="row">
              <div class="input-group mt-2 col">
                ...
                <input type="number" class="form-control qty_input form-control-sm text-center" value="<?= $cart->qty ?>" min="1" name="product_qty[<?= $idx ?>]">
          ...
          <div class="pe-2 py-2 my-1 text-end d-flex flex-column justify-content-between">
            <h6 class="color-esensia mb-0">Rp. <?= number_format(($cart->price * $cart->qty), 0, '', '.') ?></h6>
            <!-- <small class="discount">Rp. 100000</small> -->
            <a href="delete/<?= $cart->id ?>" class="px-1 py-2 ms-auto" style="width:fit-content;"><i class="fas fa-trash text-danger fa-xl"></i></a>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
